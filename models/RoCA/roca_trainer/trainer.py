...
from models.RoCA.roca_trainer.early_stopping import EarlyStopping

sys.path.append("../../RoCA")
def Trainer(model, model_optimizer, train_dl, val_dl, test_dl, device, config, idx):
    ...
    for epoch in range(1, config.num_epoch + 1):
        # Train and validate
        train_target, train_score, train_loss = model_train(model, model_optimizer, train_dl, center,
                                                                    config, device, epoch)
        val_target, val_score_origin, val_loss, all_projection = model_evaluate(model, val_dl, center, config,
                                                                            device, epoch)
        test_target, test_score_origin, test_loss, all_projection = model_evaluate(model, test_dl, center,
                                                                                   config, device, epoch)
...
def model_train(model, model_optimizer, train_loader, center, config, device, epoch):
    ...
    for batch_idx, (data, target, aug1, aug2) in enumerate(train_loader):
        ...
        loss_oc, loss_sigam, loss_n, loss_a = train(feature1, feature_dec1, center, epoch, config, device)
...
def model_evaluate(model, test_dl, center, config, device, epoch):
    model.eval()
    total_loss, total_f1, total_precision, total_recall = [], [], [], []
    all_target, all_predict = [], []
    all_projection = []
...
def train(feature1, feature_dec1, center, epoch, config, device):
