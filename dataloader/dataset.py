...
def data_generator1(train_data, test_data, train_labels, test_labels, seed, configs, idx):
    ...
    _, val_x, _, val_y = train_test_split(train_x, train_y_window, test_size=0.2, shuffle=True, random_state=seed,
                                          stratify=train_y_window)
    ...
    # test_drop = True if configs.dataset == 'UCR' else False
    test_drop = True if idx == 232 else False
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=configs.batch_size,
                                              shuffle=False, drop_last=test_drop,
                                              num_workers=0)
