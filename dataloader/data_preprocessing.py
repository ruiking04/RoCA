...
def wadi_visual():
    ...
    # print(test_df.columns)
    test_df = np.array(test_df.set_index('Row '))
    test_data_visual = test_df[:, :127]
    train_data, test_data = norm(train_data, test_data_visual)
    # variances = np.var(test_data, axis=0)

    train_labels = train_df[:, 127]
    test_labels = test_df[:, 127]

    return train_data, test_data, test_data_visual, train_labels, test_labels
