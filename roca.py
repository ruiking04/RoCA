...
parser.add_argument('--selected_dataset', default='IOpsCompetition', type=str,
                    help='Dataset of choice: IOpsCompetition, UCR, NAB, SWaT, WADI, SMAP')
parser.add_argument('--selected_model', default='RoCA', type=str,
                    help='Model of choice: RoCA,')
...
if method == 'RoCA':
    from models.RoCA.roca_trainer.trainer import Trainer
    from models.RoCA.roca_network.model import base_Model

exec(f'from conf.roca.{data_type}_Configs import Config as Configs')
configs = Configs()
...
for idx in tqdm(range(model_num)):
    ...
    print("Data loaded ...")
    train_dl, val_dl, test_dl, test_anomaly_window_num = data_generator1(train_data, test_data, train_labels, test_labels, SEED, configs, idx)
